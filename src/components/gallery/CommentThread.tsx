...
export default function CommentThread({ photoId, initialComments }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <h3 className="font-body font-semibold text-sm text-gray-700">Comments</h3>

      {comments.length === 0 ? (
        <p className="font-body italic text-sm text-gray-400">No comments yet.</p>
      ) : (
        <ul className="space-y-3 max-h-48 overflow-y-auto pr-1">
          {comments.map(c => (
            <li key={c.id}>
              <div className="flex items-baseline gap-2">
                <span className="font-body font-semibold text-xs text-gray-700">{c.author}</span>
                <span className="font-body text-xs text-gray-400">{relativeTime(c.created_at)}</span>
              </div>
              <p className="font-body text-sm text-gray-600 mt-0.5">{c.body}</p>
            </li>
          ))}
        </ul>
      )}
